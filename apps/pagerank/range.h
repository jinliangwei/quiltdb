...
class Range {
 public:
  ...
  bool SubsetEq(const Range<T>& v) const {
    return (start_ >= v.start_ && v.end_ >= end_);
  }
  bool SupsetEq(const Range<T>& v) const {
      return (start_ <= v.start_ && v.end_ <= end_);
    }
  // return true if v \in [start, end)
  bool In(const T& v) const { return (start_ <= v && v < end_); }
  ...
  std::string ToString() { return ("[" + std::to_string(start_) + "," +
                              std::to_string(end_) + "]"); }
 private:
  T start_;
  T end_;
